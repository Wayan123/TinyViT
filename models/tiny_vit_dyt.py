...
###########################################################
# Dynamic Tanh (DyT)
# Sebagai pengganti LayerNorm, tanpa mengubah struktur yang lain
###########################################################
class DyT(nn.Module):
    def __init__(self, normalized_shape, init_alpha=0.5):
        super().__init__()
        if isinstance(normalized_shape, int):
            dim = normalized_shape
        else:
            # jika normalized_shape adalah tuple/list, ambil produknya atau elemen pertama
            # sesuai layernorm yang lazim
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None,
        ...
        ###########################################################
        # Ganti nn.LayerNorm -> DyT, tapi argumennya sama.
        ###########################################################
        self.norm = DyT(in_features)
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # Ganti nn.LayerNorm -> DyT
        ###########################################################
        self.norm_head = DyT(embed_dims[-1])
        self.head = nn.Linear(
