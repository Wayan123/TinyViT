...
from timm.models.layers import LayerNorm2d  # agar sinkron dgn dynamic_tanh
...
def convert_ln_to_dyt(module):
    """
    Mengganti setiap instance LayerNorm 
    menjadi DynamicTanh secara rekursif.
    """
    module_output = module
    if isinstance(module, nn.LayerNorm):
        # Apakah ini 2D norm atau bukan
        is_2d = not isinstance(module, LayerNorm2d)
        module_output = DynamicTanh(module.normalized_shape, is_2d)
...
################################################################################
# Mlp: Ganti nn.LayerNorm => DynamicTanh
################################################################################
class Mlp(nn.Module):
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            abias = self.attention_biases[:, self.attention_bias_idxs]
        else:
            abias = self.ab

        attn = (q @ k.transpose(-2, -1)) * self.scale + abias
...
class TinyViTBlock(nn.Module):
    r"""
    TinyViT Block (Tidak diubah strukturnya)
    """
    ...
    def forward(self, x):
        ...
        if (H == self.window_size) and (W == self.window_size):
            ...
            if padding:
                x_reshape = x_reshape[:, :H, :W].contiguous()
...
class BasicLayer(nn.Module):
    """
    A basic TinyViT layer for one stage. (Struktur sama)
    """
...
class TinyViT(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            if (m.bias is not None) and isinstance(m, nn.Linear):
